    ...
    transactions.sort((a, b) => {
        switch (sortOption) {
            ...
            case 'byAmountLH':  // low to high price
                return Math.abs(parseFloat(a.querySelector('td:nth-child(5)').innerText.substring(1).replace(/,/g, ''))) - 
                       Math.abs(parseFloat(b.querySelector('td:nth-child(5)').innerText.substring(1).replace(/,/g, '')));
            
            case 'byAmountHL':  // high to low price
                return Math.abs(parseFloat(b.querySelector('td:nth-child(5)').innerText.substring(1).replace(/,/g, ''))) - 
                       Math.abs(parseFloat(a.querySelector('td:nth-child(5)').innerText.substring(1).replace(/,/g, '')));
